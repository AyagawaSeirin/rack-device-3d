async page => {
  ...
  for(let index=0;index<views.length;index++){
    const view=views[index];current=`three/web/${view}`;
    const url=`http://127.0.0.1:8794/qa/viewers/three.html?view=${view}&variant=web&labels=1&bg=light&load=three-web-final-${index+1}`;
    ...
    await page.screenshot({path:`/root/Project/rack-device-3d/Dell-R240-3.5inch/qa/webgl-evidence/three-web/${view}.png`,scale:'css',type:'png'});
    records.push({viewer:'three',variant:'web',view,url,viewer_info:viewerInfo,status:'PASS'});
  }
