async page => {
  ...
  for(const view of views){
    index+=1;current=`babylon/web/${view}`;
    const url=`http://127.0.0.1:8794/qa/viewers/babylon.html?view=${view}&variant=web&labels=1&bg=light&load=babylon-web-${index}`;
    ...
    await page.screenshot({path:`/root/Project/rack-device-3d/Dell-R240-3.5inch/qa/webgl-evidence/babylon-web/${view}.png`,scale:'css',type:'png'});
    records.push({load_index:index,viewer:'babylon',variant:'web',view,url,screenshot:`qa/webgl-evidence/babylon-web/${view}.png`,viewer_info:viewerInfo,status:'PASS'});
  }
