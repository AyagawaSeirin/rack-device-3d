async (page) => {
  const root='/root/Project/rack-device-3d/Dell-R730-2.5inch/qa/renders/viewer-b';
  ...
  for (const model of ['standard','web']) {
    await page.goto(`http://127.0.0.1:8123/webgl/viewer-b/?model=${model}&view=front&final=1`,{waitUntil:'load'});
    ...
    for (const view of views) {
      ...
      results.push({model,view,loaded:true,viewer:load.viewer});
    }
  }
  await page.goto('http://127.0.0.1:8123/webgl/viewer-b/?model=standard&view=front&alpha=1',{waitUntil:'load'});
  await page.waitForFunction(() => document.body.dataset.status === 'ready',null,{timeout:45000});
  for (const bg of ['light','dark']) {
    await page.evaluate((b) => {window.viewerAPI.setBackground(b);window.viewerAPI.setView('front')},bg);
    await page.waitForTimeout(220);await page.screenshot({path:`${root}/standard/front-checker-${bg}.png`});
  }
  return results;
}
