...
def normal_checks(meshes):
    expected_faces = {
        "face_front": np.asarray((0.0, 0.0, 1.0)),
        "face_rear": np.asarray((0.0, 0.0, -1.0)),
        "face_left": np.asarray((-1.0, 0.0, 0.0)),
        "face_right": np.asarray((1.0, 0.0, 0.0)),
        "face_top": np.asarray((0.0, 1.0, 0.0)),
        "face_bottom": np.asarray((0.0, -1.0, 0.0)),
    }
    records, errors = [], []
    for node_name, mesh in meshes:
        ...
        degenerate = int(np.sum(magnitudes <= 1e-12))
        winding = bool(mesh.is_winding_consistent)
        orientation = None
        lowered = node_name.lower()
        for token, expected in expected_faces.items():
            if token in lowered and len(mesh.face_normals):
                mean = np.mean(np.asarray(mesh.face_normals, dtype=float), axis=0)
                norm = float(np.linalg.norm(mean))
                dot = float(np.dot(mean / norm, expected)) if norm else -1.0
                orientation = {"expected": expected.tolist(), "meanNormal": mean.tolist(), "dot": dot}
                if dot < 0.99:
                    errors.append({"node": node_name, "reason": "canonical face normal is reversed or non-axial", "dot": dot})
                break
        if not finite:
            errors.append({"node": node_name, "reason": "non-finite vertex or face normal"})
        if degenerate:
            errors.append({"node": node_name, "reason": "degenerate triangles", "count": degenerate})
        if not winding:
            errors.append({"node": node_name, "reason": "inconsistent triangle winding"})
        records.append({"node": node_name, "finite": finite, "degenerateTriangles": degenerate,
                        "windingConsistent": winding, "canonicalOrientation": orientation})
...
def audit(path: Path):
    ...
    normals, normal_errors = normal_checks(meshes)
    ...
    if normal_errors:
        errors.append("normal or winding violation")
    ...
    return {
        ...
        "normals": {"meshes": normals, "errors": normal_errors},
