...
TOL = 1e-7
MIN_SKIN_CORE_CLEARANCE_M = 0.00030
MIN_APPEARANCE_SURFACE_GAP_M = 0.00024

SKIN_DIRECTIONS = {
    "Front_SourceLocked_Texture": (2, 1),
    "Rear_SourceLocked_Texture": (2, -1),
    "Physical_Left_SourceLocked_Texture": (0, -1),
    "Physical_Right_SourceLocked_Texture": (0, 1),
    "Top_SourceLocked_Texture": (1, 1),
    "Bottom_GenericFallback_Texture": (1, -1),
}
...
def skin_core_clearance_checks(meshes):
    by_name = {name: mesh for name, mesh in meshes}
    core = by_name.get("Closed_2U_Chassis_Core")
    records = []
    violations = []
    if core is None:
        return records, [{"node": "Closed_2U_Chassis_Core", "reason": "missing core"}]
    for name, (axis, sign) in SKIN_DIRECTIONS.items():
        mesh = by_name.get(name)
        if mesh is None:
            violations.append({"node": name, "reason": "missing appearance skin"})
            continue
        coordinate = float(np.mean(mesh.vertices[:, axis]))
        core_coordinate = float(core.bounds[1][axis] if sign > 0 else core.bounds[0][axis])
        clearance = float((coordinate - core_coordinate) * sign)
        triangles = np.asarray(mesh.triangles)
        normal = np.cross(triangles[0][1] - triangles[0][0], triangles[0][2] - triangles[0][0])
        normal /= np.linalg.norm(normal)
        normal_outward = bool(float(normal[axis]) * sign > 0.999)
        outside_with_clearance = bool(clearance >= MIN_SKIN_CORE_CLEARANCE_M - TOL)
        record = {
            "node": name,
            "axis": axis,
            "outwardSign": sign,
            "coordinateM": coordinate,
            "coreCoordinateM": core_coordinate,
            "clearanceM": clearance,
            "clearanceMm": clearance * 1000.0,
            "requiredClearanceMm": MIN_SKIN_CORE_CLEARANCE_M * 1000.0,
            "normalOutward": normal_outward,
            "outsideCoreWithClearance": outside_with_clearance,
        }
        records.append(record)
        if not normal_outward or not outside_with_clearance:
            violations.append(record)
    return records, violations


def near_coplanar_appearance_checks(grouped):
    surfaces = []
    for (axis, coordinate), entries in grouped.items():
        for (node, sign), rect in entries.items():
            surfaces.append((axis, coordinate, node, sign, rect))
    records = []
    appearance_tokens = ("source", "texture", "canonical", "appearance", "approved_imagegen")
    for left in range(len(surfaces)):
        axis_a, coord_a, node_a, sign_a, rect_a = surfaces[left]
        for right in range(left + 1, len(surfaces)):
            axis_b, coord_b, node_b, sign_b, rect_b = surfaces[right]
            if axis_a != axis_b or sign_a != sign_b or node_a == node_b:
                continue
            separation = abs(coord_a - coord_b)
            if separation <= TOL or separation >= MIN_APPEARANCE_SURFACE_GAP_M - TOL:
                continue
            if not any(token in (node_a + node_b).lower() for token in appearance_tokens):
                continue
            ...
            records.append({
                "nodes": [node_a, node_b],
                "axis": axis_a,
                "planesM": [coord_a, coord_b],
                "separationM": separation,
                "separationMm": separation * 1000.0,
                "minimumAllowedMm": MIN_APPEARANCE_SURFACE_GAP_M * 1000.0,
                "projectedOverlapArea": area,
            })
    return sorted(records, key=lambda value: (value["separationM"], -value["projectedOverlapArea"]))
...
def audit(path: Path):
    ...
    for node_name, mesh in meshes:
        ...
        triangles = np.asarray(mesh.triangles)
        cross = np.cross(triangles[:, 1] - triangles[:, 0], triangles[:, 2] - triangles[:, 0]) if len(triangles) else np.empty((0, 3))
        double_areas = np.linalg.norm(cross, axis=1) if len(cross) else np.asarray([])
        degenerate = int(np.sum(double_areas <= 1e-12))
        finite = bool(np.all(np.isfinite(mesh.vertices)) and np.all(np.isfinite(cross)))
        winding = bool(mesh.is_winding_consistent)
        welded_for_normals = trimesh.Trimesh(vertices=mesh.vertices.copy(), faces=mesh.faces.copy(), process=True)
        positive_volume = None if not welded_for_normals.is_watertight else bool(welded_for_normals.volume > 0)
        record = {
            "node": node_name,
            "triangleCount": int(len(triangles)),
            "degenerateTriangles": degenerate,
            "finiteGeometryAndNormals": finite,
            "windingConsistent": winding,
            "watertight": bool(welded_for_normals.is_watertight),
            "positiveVolumeWhenWatertight": positive_volume,
        }
        normal_checks.append(record)
        if degenerate or not finite or not winding or positive_volume is False:
            normal_violations.append(record)
    ...
    grouped_surfaces = axis_surfaces(meshes)
    same_facing_coplanar = defaultdict(float)
    opposite_facing_contacts = defaultdict(float)
    for (axis, coord), entries in grouped_surfaces.items():
    ...
    near_appearance_records = near_coplanar_appearance_checks(grouped_surfaces)
    opposite_records = [
    ...
    skin_clearance_records, skin_clearance_violations = skin_core_clearance_checks(meshes)
    ...
    if near_appearance_records:
        errors.append("near-coplanar appearance-surface overlap")
    if skin_clearance_violations:
        errors.append("appearance skin is inside core, inward-facing, or below minimum clearance")
    if not core_meshes or core_errors:
        errors.append("closed-core check failed")
    if normal_violations:
        errors.append("normal/winding/degenerate-geometry violation")
    ...
    return {
        ...
        "nearCoplanar": {"minimumAppearanceSurfaceGapM": MIN_APPEARANCE_SURFACE_GAP_M, "appearanceSurfaceOverlaps": near_appearance_records[:500]},
        "skinCoreClearance": {"minimumM": MIN_SKIN_CORE_CLEARANCE_M, "checks": skin_clearance_records, "violations": skin_clearance_violations},
        "closedCore": {"candidates": core_meshes, "errors": core_errors},
        "normalChecks": {"meshes": normal_checks, "violations": normal_violations},
