...
def textured_quad(face: str, image: Image.Image) -> trimesh.Trimesh:
    ...
    if face == "front":
        x0, x1, z = -INSTALLED_W / 2.0, INSTALLED_W / 2.0, OUTER_FRONT - 0.00012
        verts = [[x0, iy0, z], [x1, iy0, z], [x1, iy1, z], [x0, iy1, z]]
    elif face == "rear":
        z = OUTER_REAR + 0.00012
        verts = [[bx1, iy0, z], [bx0, iy0, z], [bx0, iy1, z], [bx1, iy1, z]]
    elif face == "left":
        verts = [[bx0, iy0, bz0], [bx0, iy0, bz1],
                 [bx0, iy1, bz1], [bx0, iy1, bz0]]
    elif face == "right":
        verts = [[bx1, iy0, bz1], [bx1, iy0, bz0],
                 [bx1, iy1, bz0], [bx1, iy1, bz1]]
    elif face == "top":
        verts = [[bx0, iy1, bz1], [bx1, iy1, bz1],
                 [bx1, iy1, bz0], [bx0, iy1, bz0]]
    elif face == "bottom":
...
def build_rear(scene: trimesh.Scene, sections: int) -> None:
    ...
    for sled, x, y, label in sled_specs:
        ...
        add_group(scene, f"Rear_C6420_Sled_{sled}_{label}_Boundary",
                  frame_bars(x, y, side_w - 0.0010, module_h - 0.0010,
                             face_z, 0.00020, 0.00065, MAT_LIGHT), MAT_LIGHT)
        ...
        usb = [
            make_box([0.0090, 0.0032, 0.00020],
                     [port_base_x - 0.006, y + 0.004, face_z], MAT_BLUE),
            make_box([0.0090, 0.0032, 0.00020],
                     [port_base_x - 0.006, y - 0.002, face_z], MAT_BLUE),
        ]
    ...
    for number, y in ((1, module_h / 2.0), (2, -module_h / 2.0)):
        ...
        add_group(scene, f"Rear_Shared_AC_EPP1600W_PSU_{number}_IEC_C14",
                  frame_bars(0.021, y, 0.027, 0.017, face_z,
                             0.00020, 0.00075, MAT_BLACK), MAT_BLACK)
        add_group(scene, f"Rear_Shared_AC_EPP1600W_PSU_{number}_Fan_Cavity",
                  [make_torus(0.0108, 0.0010,
                              [-0.020, y, OUTER_REAR + 0.0010], MAT_DARK,
                              24 if sections >= 24 else 18, 8),
                   make_cylinder(0.0030, 0.00020, [-0.020, y, face_z], MAT_DARK,
                                 sections, (0, 0, 1))], MAT_DARK)
        grille = []
        for offset in np.linspace(-0.010, 0.010, 5):
            grille.append(make_box([0.0010, 0.025, 0.00020],
                                   [-0.020 + float(offset), y, face_z], MAT_LIGHT))
            grille.append(make_box([0.025, 0.0010, 0.00020],
                                   [-0.020, y + float(offset), face_z], MAT_LIGHT))
        ...
        add(scene, f"Rear_Shared_AC_EPP1600W_PSU_{number}_Green_Release",
            make_box([0.009, 0.004, 0.00020], [0.039, y - 0.014, face_z], MAT_GREEN))
...
def build_side_top_relief(scene: trimesh.Scene, sections: int) -> None:
    ...
    top_y = BODY_H / 2.0 - 0.00010
    seams = []
    for z in (-0.216, 0.118, 0.302):
        seams.append(make_box([BODY_W - 0.008, 0.00020, 0.0010],
                              [0.0, top_y, z], MAT_DARK_SILVER))
    for x in (-0.164, 0.164):
        seams.append(make_box([0.0010, 0.00020, 0.268],
                              [x, top_y, 0.185], MAT_DARK_SILVER))
    ...
    for x, z in ((-0.205, 0.350), (0.205, 0.350), (-0.205, 0.020),
        ...
        screws.append(make_cylinder(0.0018, 0.00020, [x, top_y, z], MAT_LIGHT,
                                    sections, (0, 1, 0)))
