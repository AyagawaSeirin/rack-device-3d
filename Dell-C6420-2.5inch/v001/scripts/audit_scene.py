...
P=Path('/root/Blender/DELL-C6420/v001');scene=bpy.context.scene;dg=bpy.context.evaluated_depsgraph_get()
...
print(json.dumps({k:report[k] for k in ['object_count','triangle_count','bbox_mm','components']},indent=2));print('FLAGS',len(bad),[(x['name'],x['boundary_edges'],x['degenerate_faces']) for x in bad]);print('Duplicate faces',len(exact_duplicates))
