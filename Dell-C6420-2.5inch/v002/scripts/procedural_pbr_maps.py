...
import json
P=Path('/root/Project/rack-device-3d/Dell-C6420-2.5inch/v002/textures');N=2048;rng=np.random.default_rng(642002)
...
base=np.array([.785,.792,.797])[None,None,:]*(1+.012*coarse[:,:,None]+.005*spangle[:,:,None])
...
base=np.array([.125,.131,.138])[None,None,:]*(1+.025*coarse[:,:,None]+.032*grain[:,:,None])
...
u=x/12;v=y/12;checker=(np.floor(u/2)+np.floor(v/2))%2
weave=np.where(checker<1,np.sin(u*np.pi)**2,np.sin(v*np.pi)**2);fuzz=noise(.65)
base=np.array([.095,.101,.108])[None,None,:]*(.8+.22*weave[:,:,None]+.04*fuzz[:,:,None])
write('nylon',base,.79+.04*fuzz,9e-6*weave+6e-7*fuzz,.0256)
