async (page) => {
  ...
  const views = ["front-right", "rear-left", "rear-right"];
  ...
  await page.goto(`${root}/qa/viewer-threejs/index.html?view=front&background=dark&model=${model}`, { waitUntil: "load", timeout: 120000 });
  await page.waitForFunction(() => window.qaReady === true, null, { timeout: 120000 });
  await page.screenshot({ path: `${output}/web-front-dark.png`, type: "png" });
  return { engine: "Three.js", profile: "web", captures: [...views, "front-dark"] };
}
