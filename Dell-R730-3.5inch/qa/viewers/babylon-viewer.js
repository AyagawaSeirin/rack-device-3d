(() => {
  ...
  const scene=new BABYLON.Scene(engine); scene.useRightHandedSystem=true; scene.clearColor=new BABYLON.Color4(.694,.718,.745,1);
  const hemi=new BABYLON.HemisphericLight('hemi',new BABYLON.Vector3(0,1,0),scene);hemi.intensity=1.8;hemi.groundColor=new BABYLON.Color3(.35,.40,.45);
  ...
  const qa=window.__QA={viewer:'babylon',model:modelKey,view:viewKey,run,loaded:false,error:null};
  ...
  BABYLON.SceneLoader.ImportMeshAsync('', '../../model/', file, scene).then(async result=>{
    ...
    for(const m of result.meshes){if(!m.getBoundingInfo)continue;m.computeWorldMatrix(true);const b=m.getBoundingInfo().boundingBox;min=BABYLON.Vector3.Minimize(min,b.minimumWorld);max=BABYLON.Vector3.Maximize(max,b.maximumWorld)}
    ...
    const cam=new BABYLON.FreeCamera('qa-camera',center.add(dir.scale(2)),scene);cam.upVector=up;cam.setTarget(center);cam.minZ=.001;cam.maxZ=10;
    if(spec.o){
      cam.mode=BABYLON.Camera.ORTHOGRAPHIC_CAMERA;
      const viewDir=dir.scale(-1),right=BABYLON.Vector3.Cross(viewDir,up).normalize();
      const corners=[];for(const x of [min.x,max.x])for(const y of [min.y,max.y])for(const z of [min.z,max.z])corners.push(new BABYLON.Vector3(x,y,z));
      let minR=Infinity,maxR=-Infinity,minU=Infinity,maxU=-Infinity;
      for(const p of corners){const d=p.subtract(center),r=BABYLON.Vector3.Dot(d,right),u=BABYLON.Vector3.Dot(d,up);minR=Math.min(minR,r);maxR=Math.max(maxR,r);minU=Math.min(minU,u);maxU=Math.max(maxU,u)}
      let w=(maxR-minR)*1.12,h=(maxU-minU)*1.18;const aspect=engine.getRenderWidth()/engine.getRenderHeight();if(w/h<aspect)w=h*aspect;else h=w/aspect;
      cam.orthoLeft=-w/2;cam.orthoRight=w/2;cam.orthoTop=h/2;cam.orthoBottom=-h/2;
    }else{
      cam.fov=28*Math.PI/180;const radius=size.length()/2;const dist=radius/Math.sin(cam.fov/2)*1.18;cam.position=center.add(dir.scale(dist));cam.setTarget(center);
    }
    scene.activeCamera=cam;engine.runRenderLoop(()=>scene.render());scene.render();await new Promise(requestAnimationFrame);scene.render();
  ...
  }).catch(err=>{qa.error=String(err);document.querySelector('#status').textContent='ERROR';});
