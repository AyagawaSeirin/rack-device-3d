...
function corners(box){
  const a=[];
  for(const x of [box.min.x,box.max.x])for(const y of [box.min.y,box.max.y])for(const z of [box.min.z,box.max.z])a.push(new THREE.Vector3(x,y,z));
  return a;
}
...
function makeCamera(box,spec){
  ...
  if(spec.ortho){
    ...
    for(const p of corners(box)){
      const d=p.clone().sub(center),r=d.dot(right),u=d.dot(up);
...
const qa = window.__QA = {viewer:'three',model:modelKey,view:viewKey,run,loaded:false,error:null,modelFile:file};
new GLTFLoader().load(`../../model/${file}`, async (gltf)=>{
  ...
  renderer.render(scene,camera);
  qa.loaded=true;
  qa.bounds={min:box.min.toArray(),max:box.max.toArray(),dimensions:box.getSize(new THREE.Vector3()).toArray()};
  qa.nodes=root.children.length;
  qa.readyAt=new Date().toISOString();
  document.body.dataset.ready='true';
  document.body.dataset.engine='threejs';
