async page => {
  ...
  const root='/root/Project/rack-device-3d/Juniper-MX204/qa/revalidation-2026-08-24/renders/babylon';
  ...
  for(const model of models){
    for(const view of views){
      const loadId=`${String(records.length+41).padStart(2,'0')}-babylon-corrected-${model.variant}-${view}`;
      const url=`${base}/babylon.html?model=${encodeURIComponent(model.path)}&expected=${model.sha256}&view=${encodeURIComponent(view)}&load_id=${encodeURIComponent(loadId)}`;
      ...
      const screenshot=`${root}/${model.variant}/${view}.png`;
      await page.screenshot({path:screenshot,type:'png'});
      records.push({...result,variant:model.variant,right_handed_scene:true,screenshot,timestamp:new Date().toISOString()});
    }
  }
  return {
    status:'PASS',
    repair:'Babylon scene.useRightHandedSystem=true; prior left-handed captures excluded',
    ...
    engine_variant_counts:{'babylon-standard':records.filter(r=>r.variant==='standard').length,'babylon-web':records.filter(r=>r.variant==='web').length},
    records
  };
}
