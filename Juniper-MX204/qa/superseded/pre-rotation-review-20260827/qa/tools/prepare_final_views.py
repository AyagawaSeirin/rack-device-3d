...
top-label pseudo-text, and an independent right-source side-rail count repair.
"""
...
def repair_right_rail(right: Image.Image) -> Image.Image:
    """Remove only the spurious fourth divider from the independent right face.

    The right-side imagegen call produced the correct right-hand orientation and
    material, but split the verified rear rail section into two panels. Official
    installation figures prove three sections.  Drop the narrow separator strip
    between those two rear pieces and join the remaining pixels; no left-side
    pixels, horizontal flip, or mirrored derivative enters the right asset.
    """
    # The unwanted divider occupies roughly 77%-86% of the tightly cropped
    # source, including the outgoing right border and incoming left border.
    split_start = round(right.width * 0.768)
    split_end = round(right.width * 0.862)
    repaired = Image.new(
        "RGBA",
        (right.width - (split_end - split_start), right.height),
        (0, 0, 0, 0),
    )
    repaired.alpha_composite(right.crop((0, 0, split_start, right.height)), (0, 0))
    repaired.alpha_composite(
        right.crop((split_end, 0, right.width, right.height)),
        (split_start, 0),
    )
    return repaired
...
def main() -> None:
    ...
    raw["right"] = repair_right_rail(raw["right"])
