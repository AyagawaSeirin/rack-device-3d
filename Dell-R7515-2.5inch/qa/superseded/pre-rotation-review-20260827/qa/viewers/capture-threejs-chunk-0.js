async page => {
  const entries = [['standard','front',21],['standard','rear',22],['standard','left',23]];
  const results = [];
  for (const [model, view, sequence] of entries) {
    ...
    await page.waitForFunction(() => window.__qa?.ready === true, null, { timeout: 60000 }); await page.waitForTimeout(400);
    const qa = await page.evaluate(() => window.__qa); const screenshot = `/root/Project/rack-device-3d/Dell-R7515-2.5inch/qa/renders/threejs/${model}/${view}.png`;
