...
STANDARD_NAME = "Lenovo-ThinkServer-SR655-3.5inch.glb"
WEB_NAME = "Lenovo-ThinkServer-SR655-3.5inch-web.glb"
...
def add_front_geometry(scene: trimesh.Scene) -> None:
    ...
    gap_x = 0.004
    gap_y = 0.003
    cell_w = (region_max - region_min - 3 * gap_x) / 4.0
    cell_h = (0.066 - 2 * gap_y) / 3.0
    y_top = 0.033 - cell_h / 2.0
    z_face = Z_MAX
    for col in range(4):
        x = region_min + cell_w / 2.0 + col * (cell_w + gap_x)
        red_x = region_min + col * (cell_w + gap_x) + 0.0012
        add_box(scene, f"FrontColumnRedAccent_{col+1}", (0.0022, 0.068, 0.005), (red_x, 0.0, z_face - 0.0025), "red")
        for row in range(3):
            y = y_top - row * (cell_h + gap_y)
            idx = row * 4 + col + 1
            # Four-sided carrier frame leaves the photo texture visible through
            # the center while adding real depth, parallax and shadow.
            frame_w = cell_w * 0.98
            frame_h = cell_h * 0.96
            bar = 0.0022
            depth = 0.011
...
def add_side_top_bottom_geometry(scene: trimesh.Scene) -> None:
    # Right side: exact boss pattern and upper structural lip.
    right_x = BODY_W / 2.0
    left_x = -BODY_W / 2.0
    for side, x, axis in (("Right", right_x, "x"), ("Left", left_x, "x")):
        add_box(scene, f"{side}UpperRailLip", (0.005, 0.007, 0.590), (x, 0.031, -0.015), "light_silver")
        for idx, z in enumerate((0.255, 0.078, -0.105, -0.275), 1):
            add_cylinder(scene, f"{side}RaisedBoss{idx}", 0.0040, 0.004, (x, -0.010, z), "light_silver", axis=axis, sections=24)
        screw_positions = ((0.026, 0.300), (0.015, 0.220), (0.022, 0.020), (0.018, -0.170), (-0.018, -0.310))
        for idx, (y, z) in enumerate(screw_positions, 1):
            add_cylinder(scene, f"{side}Screw{idx}", 0.00125, 0.0044, (x, y, z), "dark_silver", axis=axis, sections=16)
