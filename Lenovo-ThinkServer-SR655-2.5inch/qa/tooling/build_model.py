...
def add_front_geometry(scene: trimesh.Scene) -> None:
    ...
    for idx in range(24):
        ...
        add_box(scene, f"{name}_Left", (side_bar, frame_h, depth), (x - frame_w / 2.0 + side_bar / 2.0, y, center_z), "black")
        add_box(scene, f"{name}_Right", (side_bar, frame_h, depth), (x + frame_w / 2.0 - side_bar / 2.0, y, center_z), "black")
...
def add_side_top_bottom_geometry(scene: trimesh.Scene) -> None:
    ...
    add_box(scene, "RightWarningLabelRelief", (0.0020, 0.012, 0.034), (right_x, 0.004, 0.205), "yellow")
