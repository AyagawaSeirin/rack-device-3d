...
$('#closeMenu').click(function () {
  let width = $('.left-menu-container').width();
  $('.left-menu-container').animate(
    {
      right: '-' + width + 'px',
    },
  ...
  if ($('#menu-item-box2').hasClass('active')) {
    ...
    $('#menu-item-box2')
      .animate(
        ...
        300,
        function () {
          $('#menu-item-box2').css({ display: 'none' });
        }
      )
      .removeClass('active');
    $('.left-menu').css({
...
$('#menu-item-box1>.menu-item>input').on('click', function (e) {
  //监听radio，实现radio的选中与取消

  if ($(e.target).attr('checked') == 'checked') {
    //如果之前状态为已选中，则取消选中，并且删除nodeList中对应的存储的值，
...
function originView() {
  ...
  view3d.playAnimation({ speed: 1, type: 1, from: 1, to: 0 });
  setTimeout(function () {
  ...
  if (browserType == 'mobile') {
    $("[i18n='main.text']").css({ display: '-webkit-box' });
    $("[i18n='main.title']").css({ display: '-webkit-box' });
  }
}
function boomView() {
  view3d.setDefView(iv.VIEW_TRANSITION);
  ...
  if (iv.setEditMode) {
    iv.setEditMode("select");
  }
  if (browserType == 'mobile') {
    $("[i18n='main.text']").css({ display: 'none' });
    $("[i18n='main.title']").css({ display: 'none' });
  }
}
