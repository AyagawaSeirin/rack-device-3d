function click_component() {
  ...
  view3d.addRefTarget(function (event) {
    switch (event.code) {
      case 'selection':
        {
          if (move_flag == 1) {
            ...
            if (boom_flag == 1) {
              ...
              if (marks.indexOf(event.node.name) != -1) {
                //如果是点的话 设置选中的颜色为本来的颜色
                for (var i = 0; i < marks.length; i++) {
                  if (marks[i] == event.node.name) {
                    nodeColor = [1, 0.5122, 0.5122, 0.5, 1, 1];
                    return;
                  }
                }
              } else {
            ...
            if (marks.indexOf(event.node.name) != -1) {
              ...
              nodeColor = [
                event.node.material.diffuse[0].color[0] || 1,
                event.node.material.diffuse[0].color[1] || 0,
                event.node.material.diffuse[0].color[2]  || 0,
                0.5,
                1,
                1,
              ]; //如果不是点，则设置颜色为材质颜色
      ...
      case 'mousedown':
        {
          ...
          if (browserType == 'mobile') {
            hitNode(event.x, event.y, event.node);
          }
        }
        break;
    }
...
const caseHover = throttle(function(event) {
  hoverNode(event.x, event.y)
}, 20);
